...
export default function BreakEvenPage() {
  ...
  // Re-type check for error handling
  const hasError = analysis && 'error' in analysis;
  const result = hasError ? null : analysis;

  const chartData = [
    { name: 'Actual Revenue', value: totalRevenue, color: '#3B82F6' },
    { name: 'Break-Even Point', value: result?.breakEvenPoint || 0, color: '#0F172A' },
  ];

  const survivalProgress = result ? Math.min((totalRevenue / result.breakEvenPoint) * 100, 100) : 0;
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#0F172A] p-8 rounded-3xl text-white relative overflow-hidden">
        ...
        <div className="relative z-10 flex gap-4">
          {result && (
            <Badge className={`px-4 py-1.5 text-sm font-bold uppercase tracking-widest ${result.isProfitable ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>
              {result.isProfitable ? 'Self-Sustaining' : 'Burn Phase'}
            </Badge>
          )}
        </div>
      </div>
      ...
      {hasError ? (
        <Card className="border-none shadow-xl bg-rose-50 border border-rose-100">
          <CardContent className="p-12 text-center space-y-4">
            <AlertTriangle className="h-12 w-12 text-rose-500 mx-auto" />
            <h3 className="text-xl font-bold text-rose-900">Break-Even Impossible</h3>
            <p className="text-rose-700 max-w-md mx-auto">{(analysis as any).error}</p>
          </CardContent>
        </Card>
      ) : (
        <>
            ...
            <Card className="lg:col-span-1 border-none shadow-xl bg-white overflow-hidden flex flex-col justify-center text-center p-8">
              <CardHeader className="p-0 mb-6">
                ...
                <CardTitle className="text-4xl font-bold text-slate-900 font-headline">
                  {result ? formatINR(result.breakEvenPoint) : '---'}
                </CardTitle>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className={`text-2xl font-bold ${result?.isProfitable ? 'text-emerald-600' : 'text-rose-600'}`}>
                        {result ? `${result.marginOfSafety}%` : '---'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-6 rounded-2xl bg-white/10 border border-white/20 backdrop-blur-sm">
                    ...
                    <p className="text-lg italic leading-relaxed">
                      {result?.isProfitable 
                        ...
                        : result?.gap && result.gap > (totalRevenue * 0.5) 
