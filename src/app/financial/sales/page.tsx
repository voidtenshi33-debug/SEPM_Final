...
export default function SalesPage() {
  ...
  const getAISuggestion = () => {
    ...
    if (businessType === "Product") {
      if (productMetrics.aov < 1000 && productMetrics.aov > 0) return "AOV is low. Suggest bundling high-margin items to improve unit economics.";
      if (latestMonth.unitsSold < 100) return "Inventory velocity is slow. Prioritize growth marketing for flagship SKU.";
    } else if (businessType === "Service") {
      if (serviceMetrics.utilizationRate < 60) return "Team utilization is low (< 60%). Significant capacity is currently 'benched'. Seek short-term consulting contracts.";
      if (serviceMetrics.clientRetention < 80) return "Client churn alert. Implement feedback loops to identify post-delivery friction.";
    }
    ...
    <div className="space-y-6 animate-in slide-in-from-left-4 duration-500">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="border-none shadow-md bg-white">
          <CardContent className="p-6">
            ...
            <p className="text-2xl font-bold font-headline text-slate-900">{formatINR(productMetrics.aov)}</p>
        ...
        <Card className="border-none shadow-md bg-white">
          <CardContent className="p-6">
            ...
            <p className="text-2xl font-bold font-headline text-slate-900">{formatINR(productMetrics.revenuePerUnit)}</p>
            <p className="text-[10px] text-muted-foreground mt-2 uppercase font-medium">{latestMonth?.unitsSold || 0} Units Sold</p>
          </CardContent>
        </Card>
        <Card className="border-none shadow-md bg-white">
          <CardContent className="p-6">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Daily Velocity</p>
            <p className="text-2xl font-bold font-headline text-slate-900">{productMetrics.dailyOrderAvg.toFixed(1)} <span className="text-sm font-medium text-muted-foreground">orders/day</span></p>
            <p className="text-[10px] text-muted-foreground mt-2 uppercase font-medium">30-Day Avg</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-none shadow-xl bg-white">
        <CardHeader>
          ...
          <CardDescription>Product sales volume trend over the last 12 months.</CardDescription>
        </CardHeader>
    ...
    <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="border-none shadow-md bg-white">
          <CardContent className="p-6">
            ...
            <p className="text-2xl font-bold font-headline text-slate-900">{serviceMetrics.utilizationRate.toFixed(1)}%</p>
        ...
        <Card className="border-none shadow-md bg-white">
          <CardContent className="p-6">
            ...
            <p className="text-2xl font-bold font-headline text-slate-900">{formatINR(serviceMetrics.revenuePerClient)}</p>
        ...
        <Card className="border-none shadow-md bg-white">
          <CardContent className="p-6">
            ...
            <p className="text-2xl font-bold font-headline text-slate-900">{serviceMetrics.clientRetention.toFixed(1)}%</p>
            <p className="text-[10px] text-muted-foreground mt-2 uppercase font-medium">Active Continuity</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-none shadow-xl bg-white">
        <CardHeader>
          ...
          <CardDescription>Pipeline Value vs. Closed Revenue comparison.</CardDescription>
        </CardHeader>
        <CardContent className="h-[350px]">
          {mounted && (
            <ResponsiveContainer width="100%" height="100%">
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2 text-slate-900">
            ...
            Sales Intelligence: {businessType} Model
          </h2>
          <p className="text-sm text-muted-foreground">Unified tracking for your core business engine (INR).</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 space-y-8">
          {businessType === "Product" && renderProductView()}
          {businessType === "Service" && renderServiceView()}
          {businessType === "Hybrid" && (
        ...
        <div className="space-y-6">
          <Card className="border-none shadow-xl bg-slate-950 text-white">
            <CardHeader>
              <CardTitle className="text-lg flex items-center gap-2">
            ...
            <CardContent className="space-y-6">
              <div className="p-4 bg-white/10 rounded-xl border border-white/20 flex items-start gap-4">
                <div className="h-10 w-10 rounded-full bg-accent/20 flex items-center justify-center shrink-0">
                  <Activity className="h-5 w-5 text-accent" />
                </div>
                <div>
                  <p className="text-[10px] font-bold uppercase text-slate-400 mb-1">Growth Diagnosis</p>
              ...
              <div className="space-y-4 pt-4 border-t border-white/10">
                <h4 className="text-[10px] font-bold uppercase tracking-widest text-slate-400 flex items-center gap-2">
                  <Target className="h-4 w-4 text-accent" /> Strategic Focus Points
                </h4>
                <div className="space-y-3">
                  <div className="p-3 bg-white/5 rounded-lg border border-white/10 flex items-center justify-between text-xs font-bold">
                    <span className="flex items-center gap-2">Revenue Stability</span>
                    <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30">OPTIMAL</Badge>
                  </div>
                  <div className="p-3 bg-white/5 rounded-lg border border-white/10 flex items-center justify-between text-xs font-bold">
                    <span className="flex items-center gap-2">CAC Payback</span>
                    <Badge className="bg-amber-500/20 text-amber-400 border-amber-500/30">7.4 Months</Badge>
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <Card className="border-none shadow-xl bg-accent text-white">
            ...
            <CardContent>
              <p className="text-4xl font-bold font-headline">{formatINR(latestMonth?.netRevenue || 0)}</p>
              <p className="text-xs mt-2 opacity-80 uppercase font-bold tracking-widest">{latestMonth?.month || "No Data"}</p>
            </CardContent>
          </Card>
        </div>
      </div>
